...
1.从卫星图像生成样本
2.网格搜索SVM /随机森林参数
3.基于对象的分类后细化 ; 基于超像素的正则化的分类图
(object-based post-classification refinement
superpixel-based regularization for classification maps)
4.混淆矩阵：OA，kappa，PA，UA，AA
5.将地图另存为图像
...
class rscls:
    ...
    def train_sample_all_known(self):
        x_train, y_train = [], []
        self.locate_sample()
        _samp = self.sample
        for _cls in range(self.cls):
...
def gtcfm(pre,gt,ncl):
    ...
    for i in range(ncl):
        ...
        oa += cf[i,i]
    cfm[-1, 0] = oa/nsize  # oa
    cfm[-1, 1] = cfm[ncl,:-1].mean()  # aa
    cfm[-1, 2] = (cfm[-1, 0]-tmp1)/(1-tmp1)  # kappa
    cfm[-1, 3] = cfm[:-2,ncl].mean()  # ua
    print('oa, aa/pa, kappa, ua: ', '\n', format(cfm[-1,0],'.5'), '\n', format(cfm[-1,1],'.5'), '\n', 
             format(cfm[-1,2],'.5'), '\n',  format(cfm[-1,3],'.5'), '\n')
    return cfm
...
def gtcfm0(pre,gt,ncl):
    ...
    for i in range(gt.shape[0]):
        for j in range(gt.shape[1]):
            if gt[i,j]:  # 只有当gt有标签时，才会把该处的predict加到混淆矩阵里
                cf[pre[i,j],gt[i,j]] += 1
    tmp1 = 0
    nsize = np.sum(gt!=0)
    for j in range(ncl):
...
def make_sample(sample, label):#数据扩充
    a = np.flip(sample,1)
    b = np.flip(sample,2)
    c = np.flip(b,1)
    d = np.rot90(sample, k=1, axes=(1,2))
    e = np.rot90(sample, k=-1, axes=(1,2))
    print('0'*20)
    print(sample.shape)
    print(a.shape)
    print(b.shape)
    ...
    print(d.shape)
    print('1'*20)
    newsample = np.concatenate((a,b,c,d,e,sample),axis=0)
    newlabel = np.concatenate((label,label,label,label,label,label),axis=0)
